...
<div id="page12"><br/><br/>
                              ...
                              <tr>
                                <td height='400px' bgcolor="#f2f2f2" valign='top' style='padding:10px;color:#444;font-size:11px;'> <br/><br/>
                                  <?php
                                                                ...
                                                                $q1="SELECT count(*) from user_message WHERE t_id = '$user'";
                                                                $result1=mysql_query($q1);
                                                                $show1=mysql_fetch_array($result1);
                                                                //echo $show1;
                                                                $total=$show1[0];
                                                                //echo $total;
                                                                $qu="SELECT * from user_message WHERE t_id = '$user' ORDER by id DESC LIMIT $startrow,".showmax;
                                                                $show=mysql_query($qu);
                                                                ...
                                                                $t_id = $row['t_id'];				   
								$date = $row['date'];
                                                                $qu7="SELECT * from student WHERE id = '$st_id' LIMIT 1";
                                                                $show7=mysql_query($qu7); 
                                                                $row7=mysql_fetch_array($show7);
                                                                $name7 = $row7['name'];
     ...
     <table width='90%' align='center' border='0' bgcolor='#ffffff' style='color:#444;font-size:11px;'>
            ...
            <td valign='top' style='padding:5px;color:444;'>
               <span style='color:#444;text-decoration:none;'><b>$title</b></span><br/><br/><br/><a href='#openModal$id' style='background-color:#13A7C7;padding:3px;color:#ffffff;text-decoration:none;'>Details</a> 
				...
				<br/><h3>$title</h3>Send to - $name7, $date
				...
				<br/><h3>Update</h3>
				<p><hr style='border:0px;height:1px;background:#444;'/><br/>
                                       <form method='POST' action='user_update_message_record_script.php'>
   ...
   &nbsp; <a href='#openModal12$id' style='background-color:#13A7C7;padding:3px;color:#ffffff;text-decoration:none;'>Delete</a> &nbsp; <span style='color:#444;font-size:12px;text-decoration:none;'>$date</span>
                                               <div id='openModal12$id' class='modalDialog'>
						<div>
				<a href='#close' title='Close' class='close'>X</a>
				<br/><h3>Are you confirm to delete?</h3>
				<p><hr style='border:0px;height:1px;background:#444;'/><br/>
                                       <form method='POST' action='user_message_delete_script.php'>
...
if($page>0)
{?>
<a href="sent_messages.php?curpage=<?php echo ($page-1); ?>" style='background-color:#5898b4;padding:4px;color:#ffffff;text-decoration:none;'>prev</a>
<?php
}
else
{
...
{?>
<a href="sent_messages.php?curpage=<?php echo ($page+1); ?>" style='background-color:#5898b4;padding:4px;color:#ffffff;text-decoration:none;'>next</a>
<?php
}
echo "</td>
                                       </tr>
